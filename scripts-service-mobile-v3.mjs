...
function getImageSrcByClass(text, className) {
...
function addSectionStyle(text, className, imageUrl) {
  if (!imageUrl) return text;

  const styleValue = `style={{ "--mobile-bg": "url('${imageUrl}')" } as CSSProperties}`;

  const regex = new RegExp(
    `<section className=\\{styles\\.${className}\\}(?![^>]*--mobile-bg)([^>]*)>`,
    "g"
  );

  return text.replace(regex, `<section className={styles.${className}} ${styleValue}$1>`);
}
...
for (const slug of fs.readdirSync(servicesDir)) {
  ...
  const heroDesktop = getImageSrcByClass(page, "heroImageDesktop");
  const heroMobile = getImageSrcByClass(page, "heroImageMobile");
  const darkImage = getImageSrcByClass(page, "darkBandImage");
  const finalImage = getImageSrcByClass(page, "finalImage");
  ...
  page = addSectionStyle(page, "services", servicesBg);
  page = addSectionStyle(page, "finalCta", finalBg);
