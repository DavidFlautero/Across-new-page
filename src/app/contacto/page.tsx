...
export default function ContactoPage() {
  ...
  return (
    <>
      ...
      <main style={page}>
        <section className="contact-grid" style={wrap}>
          <div style={intro}>
            <span style={eyebrow}>{t.badge}</span>
            <h1 style={title}>{t.title}</h1>
            <p style={text}>{t.text}</p>

            <div className="phones-grid" style={phoneGrid}>
              {phones.map(([label, href, visible]) => (
                <a key={href} href={`tel:${href}`} style={phoneCard}>
                  <span style={phoneLabel}>{label}</span>
                  <strong style={phoneNumber}>{visible}</strong>
                  <small style={phoneCta}>{t.call}</small>
                </a>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={submit} style={form}>
            <h2 style={formTitle}>{t.formTitle}</h2>

            <div className="two-cols" style={twoCols}>
              <input name="name" placeholder={t.name} required minLength={2} style={input} />
              <input name="company" placeholder={t.company} required minLength={2} style={input} />
            </div>

            <div className="two-cols" style={twoCols}>
              <input name="email" type="email" placeholder={t.email} required style={input} />
              <input name="phone" placeholder={t.phone} required minLength={7} inputMode="tel" style={input} />
            </div>

            <select name="country" required defaultValue="" style={input}>
            ...
            <textarea name="message" placeholder={t.message} rows={7} required minLength={10} style={textarea} />

            <p style={legalText}>{t.legal}</p>
            ...
            {sent && <p style={success}>{t.success}</p>}
            {error && <p style={fail}>{error}</p>}
          </form>
        </section>
