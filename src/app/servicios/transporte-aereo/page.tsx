...
const copy = {
  es: {
    ...
    heroTitle: "Transporte aéreo internacional para cargas urgentes, sensibles o de alto valor.",
    heroText:
      "Coordinamos envíos aéreos internacionales con control documental, seguimiento operativo y soluciones adaptadas a cargas críticas, sensibles o de alto valor.",
        ...
        <section className={`${styles.processAccordionSection} aereoProcessPremium`}>
          ...
          <div className={styles.processAccordionActions}>
            <div>
              <strong>
                {locale === "es"
                  ? "¿Tiene una operación aérea en curso?"
                  : locale === "en"
...
<section className={styles.darkBand} data-mobile-hide-after-cert="true">
        ...
        <section className={styles.faqSection} data-aereo-faq-section="true">
          <div className={styles.faqHead}>
            ...
            <h2>
              {locale === "es"
                ? "Dudas frecuentes sobre transporte aéreo internacional."
                : locale === "en"
            ...
            <p>
              {locale === "es"
                ? "Resolvemos las principales dudas antes de iniciar una operación aérea: documentación, tiempos, seguimiento, restricciones y coordinación logística."
                : locale === "en"
                  ? "We answer the main questions before starting an air freight operation: documentation, timing, tracking, restrictions and logistics coordination."
                  : "我们解答空运操作前的主要问题：文件、时效、跟踪、限制和物流协调。"}
            </p>
          </div>
          ...
          <div className={styles.faqList}>
            {[
              [
                locale === "es" ? "¿Cuándo conviene usar transporte aéreo internacional?" : locale === "en" ? "When should I use international air freight?" : "什么时候适合使用国际空运？",
                locale === "es"
                  ? "Cuando la carga requiere velocidad, control documental y tiempos de entrega ajustados. Es ideal para mercancía urgente, repuestos críticos, productos de alto valor o envíos sensibles."
                  : locale === "en"
                    ? "When cargo requires speed, document control and tight delivery times. It is ideal for urgent goods, critical spare parts, high-value products or sensitive shipments."
                    : "当货物需要速度、文件管控和紧迫交付时间时，空运是理想选择，适用于紧急货物、关键备件、高价值产品或敏感货物。"
              ],
              [
                locale === "es" ? "¿Cómo se calcula una cotización aérea?" : locale === "en" ? "How is an air freight quote calculated?" : "空运报价如何计算？",
                locale === "es"
                  ? "Depende del origen, destino, peso real, peso volumétrico, dimensiones, tipo de mercancía, urgencia, documentación requerida y servicios adicionales como aduana o entrega final."
                  : locale === "en"
                    ? "It depends on origin, destination, actual weight, volumetric weight, dimensions, cargo type, urgency, required documentation and additional services such as customs or final delivery."
                    : "报价取决于起点、目的地、实际重量、体积重量、尺寸、货物类型、紧急程度、所需文件以及清关或末端配送等附加服务。"
              ],
              [
                locale === "es" ? "¿Qué documentos necesito para una operación aérea?" : locale === "en" ? "What documents are required for air freight?" : "空运需要哪些文件？",
                locale === "es"
                  ? "Generalmente se requiere factura comercial, packing list, información del consignatario, datos de la carga y documentos aduaneros o permisos especiales según el tipo de mercancía."
                  : locale === "en"
                    ? "Usually a commercial invoice, packing list, consignee information, cargo details and customs documents or special permits are required depending on the type of goods."
                    : "通常需要商业发票、装箱单、收货人信息、货物信息，以及根据货物类型所需的海关文件或特殊许可。"
              ],
              [
                locale === "es" ? "¿Qué es una guía aérea o AWB?" : locale === "en" ? "What is an Air Waybill or AWB?" : "什么是空运提单 AWB？",
                locale === "es"
                  ? "La guía aérea o AWB es el documento principal del transporte aéreo. Identifica la operación, la aerolínea, la carga, el origen, el destino y permite hacer seguimiento del envío."
                  : locale === "en"
                    ? "The Air Waybill or AWB is the main air transport document. It identifies the shipment, airline, cargo, origin, destination and allows shipment tracking."
                    : "空运提单 AWB 是空运的主要文件，用于识别运输、航空公司、货物、起点、目的地，并支持货物跟踪。"
              ],
              [
                ...
                locale === "es"
                  ? "Sí. Coordinamos la operación aérea junto con los documentos necesarios, gestión aduanera, seguimiento operativo y conexión con otros servicios logísticos cuando la carga lo requiere."
                  : locale === "en"
                    ? "Yes. We coordinate the air operation together with the required documents, customs management, operational tracking and connection with other logistics services when needed."
                    : "是的。我们协调空运操作、所需文件、清关管理、操作跟踪，并在需要时连接其他物流服务。"
              ],
              [
                locale === "es" ? "¿Qué tipo de carga puede transportarse por vía aérea?" : locale === "en" ? "What type of cargo can be shipped by air?" : "哪些货物可以通过空运运输？",
                locale === "es"
                  ? "Se puede transportar carga general, urgente, tecnológica, farmacéutica, repuestos, muestras comerciales y mercancía de alto valor. Algunas cargas requieren revisión especial por regulación."
                  : locale === "en"
                    ? "General, urgent, technology, pharmaceutical, spare parts, commercial samples and high-value cargo can be shipped by air. Some cargo requires special regulatory review."
                    : "可空运普通货物、紧急货物、科技产品、医药产品、备件、商业样品和高价值货物。部分货物需根据法规进行特殊审核。"
              ],
              [
                locale === "es" ? "¿Puedo hacer seguimiento de mi carga?" : locale === "en" ? "Can I track my cargo?" : "可以跟踪我的货物吗？",
                locale === "es"
                  ? "Sí. Realizamos seguimiento operativo durante el proceso y mantenemos visibilidad sobre los hitos principales: recepción, reserva, despacho, tránsito y entrega final."
                  : locale === "en"
                    ? "Yes. We provide operational tracking throughout the process and visibility over key milestones: reception, booking, dispatch, transit and final delivery."
                    : "可以。我们在整个流程中进行操作跟踪，并提供关键节点可视化：收货、订舱、发运、运输和最终交付。"
              ],
              [
                locale === "es" ? "¿Qué pasa si mi carga requiere manejo urgente o especial?" : locale === "en" ? "What if my cargo requires urgent or special handling?" : "如果货物需要紧急或特殊处理怎么办？",
                locale === "es"
                  ? "Evaluamos la urgencia, tipo de mercancía, restricciones, documentación y ruta disponible para coordinar una alternativa viable con el menor riesgo operativo posible."
                  : locale === "en"
                    ? "We evaluate urgency, cargo type, restrictions, documentation and available routes to coordinate a viable alternative with the lowest possible operational risk."
                    : "我们会评估紧急程度、货物类型、限制、文件和可用路线，以协调风险最低的可行方案。"
              ]
            ].map(([question, answer]) => (
