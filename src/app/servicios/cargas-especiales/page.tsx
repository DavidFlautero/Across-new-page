...
const copy = {
  es: {
    ...
    heroTitle: "¿Su carga no entra en el transporte estándar? Cotícela con especialistas.",
    heroText: "Planificamos rutas, permisos, equipos y manipulación para maquinaria, estructuras y carga sobredimensionada.",
  ...
  return (
    <div className="page-shell">
      ...
      <main className={styles.page}>
        <section className={styles.hero} data-service-name="cargas-especiales" data-service-hero-home="true"
      >
          <Image
      ...
      >
            <div className={styles.heroContent}>
              ...
              <div className={styles.actions}>
                <Link href="/cotizacion?servicio=cargas-especiales" className={styles.primaryBtn}>{t.primaryCta}</Link>
                <Link href="/contacto?servicio=cargas-especiales" className={styles.secondaryBtn}>{t.secondaryCta}</Link>
              </div>
            </div>
          ...
          <div className={styles.commandBar} data-service-trust="cargas-especiales">
        ...
        <section className={`${styles.processAccordionSection} aereoProcessPremium`}>
          ...
          <div className={styles.processAccordionActions}>
            ...
            <div>
              <Link href="/cotizacion?servicio=cargas-especiales">
                {locale === "es" ? "Solicitar cotización especial" : locale === "en" ? "Request air quote" : "申请空运报价"}
              </Link>

              <Link href="/contacto?servicio=cargas-especiales">
        ...
        <section className={styles.darkBand} data-mobile-hide-after-cert="true" data-mobile-darkband-image="true" style={{ "--dark-band-mobile-image": "url(\'/images/cargas-especiales-mobile/carga-especial2.png\')" } as any}>
          <div className={styles.darkBandImage}>
            <Image
              src="/images/cargas-especiales-desktop/transporte-especial-2.png"
              alt={t.bandTitle}
              fill
        ...
        <section className={styles.faqSection} data-cargas-especiales-faq-section="true">
          <div className={styles.faqHead}>
            ...
            <h2>
              {locale === "es"
                ? "Dudas frecuentes sobre cargas especiales."
                : locale === "en"
                  ? "Common questions about special cargo."
                  : "关于特殊货物的常见问题。"}
            </h2>

            <p>
              {locale === "es"
                ? "Resolvemos las principales dudas antes de coordinar una operación especial: dimensiones, permisos, ruta, seguridad, documentación y entrega final."
                : locale === "en"
                  ? "We answer the main questions before coordinating a special cargo operation: dimensions, permits, route, safety, documentation and final delivery."
                  : "我们解答特殊货物操作前的主要问题：尺寸、许可、路线、安全、文件和最终交付。"}
            </p>
          </div>
          ...
          <div className={styles.faqList}>
            {[
              [
                locale === "es" ? "¿Qué se considera una carga especial?" : locale === "en" ? "What is considered special cargo?" : "什么是特殊货物？",
                locale === "es"
                  ? "Es mercancía que por peso, dimensiones, fragilidad, valor o complejidad requiere planificación específica, permisos, equipos especiales o rutas controladas."
                  : locale === "en"
                    ? "It is cargo that due to weight, dimensions, fragility, value or complexity requires specific planning, permits, special equipment or controlled routes."
                    : "特殊货物是因重量、尺寸、易碎性、价值或复杂性而需要专门规划、许可、特殊设备或受控路线的货物。"
              ],
              [
                locale === "es" ? "¿Across coordina permisos y escoltas?" : locale === "en" ? "Does Across coordinate permits and escorts?" : "Across 是否协调许可和护送？",
                locale === "es"
                  ? "Sí. Coordinamos permisos, documentación, rutas especiales, operadores, seguros y escoltas cuando la operación lo requiere."
                  : locale === "en"
                    ? "Yes. We coordinate permits, documentation, special routes, operators, insurance and escorts when required."
                    : "是的。我们根据操作需要协调许可、文件、特殊路线、运营商、保险和护送。"
              ],
              [
                locale === "es" ? "¿Cómo se cotiza una carga especial?" : locale === "en" ? "How is special cargo quoted?" : "特殊货物如何报价？",
                locale === "es"
                  ? "Depende de medidas, peso, origen, destino, equipo requerido, ruta, urgencia, permisos, seguros y condiciones de manipulación."
                  : locale === "en"
                    ? "It depends on dimensions, weight, origin, destination, required equipment, route, urgency, permits, insurance and handling conditions."
                    : "报价取决于尺寸、重量、起点、目的地、所需设备、路线、紧急程度、许可、保险和操作条件。"
              ],
              [
                locale === "es" ? "¿Pueden coordinar cargas sobredimensionadas?" : locale === "en" ? "Can oversized cargo be coordinated?" : "可以协调超尺寸货物吗？",
                locale === "es"
                  ? "Sí. Diseñamos la operación según dimensiones, restricciones de ruta, medios de transporte, manipulación y requisitos documentales."
                  : locale === "en"
                    ? "Yes. We design the operation according to dimensions, route restrictions, transport mode, handling and documentation requirements."
                    : "可以。我们根据尺寸、路线限制、运输方式、操作和文件要求设计运输方案。"
              ],
              [
                locale === "es" ? "¿Se puede hacer seguimiento de la operación?" : locale === "en" ? "Can the operation be tracked?" : "可以跟踪操作吗？",
                locale === "es"
                  ? "Sí. Mantenemos seguimiento operativo sobre hitos críticos: preparación, retiro, tránsito, permisos, llegada y entrega final."
                  : locale === "en"
                    ? "Yes. We track key milestones: preparation, pickup, transit, permits, arrival and final delivery."
                    : "可以。我们跟踪关键节点：准备、提货、运输、许可、到达和最终交付。"
              ],
              [
                locale === "es" ? "¿Qué información debo enviar para cotizar?" : locale === "en" ? "What information is needed for a quote?" : "报价需要提供哪些信息？",
                locale === "es"
                  ? "Medidas, peso, fotos, ficha técnica, origen, destino, fecha estimada, tipo de mercancía y cualquier restricción de manipulación."
                  : locale === "en"
                    ? "Dimensions, weight, photos, technical sheet, origin, destination, estimated date, cargo type and any handling restrictions."
                    : "需要提供尺寸、重量、照片、技术资料、起点、目的地、预计日期、货物类型和任何操作限制。"
              ]
            ].map(([question, answer]) => (
...
<RelatedServices current="cargas-especiales" locale={locale} />
      ...
      <div className={styles.mobileStickyCta} aria-label="Acciones rápidas de cargas especiales">
        <Link href="/cotizacion?servicio=cargas-especiales">
