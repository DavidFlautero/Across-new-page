...
def insert_import(text: str) -> str:
    text = re.sub(
        r'^import HomeOperatorOnly from "@/components/sections/HomeOperatorOnly";\s*$',
        "",
        text,
        flags=re.M,
    )
...
def insert_after_process(text: str) -> str:
    # limpiar llamadas anteriores para no duplicar
    ...
    # fallback si alguna página no tiene process
...
for page in sorted(ROOT.glob("*/page.tsx")):
    ...
    backup = page.with_suffix(page.suffix + ".bak-before-home-operator-all")
    if not backup.exists():
        shutil.copy2(page, backup)

    # limpiar imports viejos/inventados
    ...
    # borrar bloques viejos completos
    ...
    # borrar const sectorImage si quedó sin uso
    if "sectorImage" in text:
        temp = re.sub(r'^\s*const sectorImage = .+?;\s*$', "", text, flags=re.M)
        if "sectorImage" not in temp:
            text = temp
...
print("✅ HomeOperatorOnly colocado en todos los sectores:")
for p in updated:
    print(" -", p)

print("\n✅ Se eliminaron darkBand/finalCta de los page.tsx de sectores.")
