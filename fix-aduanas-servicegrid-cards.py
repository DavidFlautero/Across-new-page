...
targets = [
    ...
    Path("src/app/servicios/aduana/Aduanas.module.css"),
]
...
if css_path is None:
    matches = list(Path("src").rglob("Aduanas.module.css"))
    if matches:
        css_path = matches[0]

if css_path is None:
    raise SystemExit("❌ No encontré Aduanas.module.css en src/")
...
marker = "/* === ADUANAS SERVICEGRID CARDS VISIBLE LOCAL FINAL === */"

patch = r'''
/* === ADUANAS SERVICEGRID CARDS VISIBLE LOCAL FINAL === */
...
if marker in css:
    css = re.sub(
        r'/\* === ADUANAS SERVICEGRID CARDS VISIBLE LOCAL FINAL === \*/[\s\S]*?(?=\n/\* ===|\Z)',
        patch.strip(),
        css,
        flags=re.M,
    )
